...
Aop=@(x) -x/2+A_operator_Lap(s,x,U,nei); % A_operator = close(closeeval) & far(fmm) vesicle interactions.
...
rhs = [0*s.x+dip(:); jumps(1)+0*L.x; 0*L.x; jumps(2)+0*B.x; 0*B.x]; % box driving
 ...
 % Ap = A - Bm*QdagC;  % Schur (in dense not fast apply form)
  ...
  sig = gmres(@(x) Aop(x)-Bm*(QdagC*x), dip(:)-Bm*Qdagg,[],1e-14,50);
...
while ls<n^2
    ...
    tt.x=t.x(ls+1:lsmax);
	[uu,uux,uuy]=At_operator_Lap(tt,s,sig,U,nei);
...
shift = jFun(1)-u(1);
figure(1), subplot(1,3,1), imagesc(X(1,:),Y(:,1),log10(abs(u+shift-jFun))), colorbar, title('u error'), axis square off
